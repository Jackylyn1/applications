...
## OUTPUT — write a single copy-paste-ready `.txt` to `career-kb/output/LinkedIn_Profile_optimized_[language].txt`
...
3. **EXPERIENCE** — EVERY genuine position, each with a keyword-rich title line + 2-4 honest, concrete, measurable bullets. Gastro = core contributor / owns specific components (external API + multi-tenant permissions, real-time pivot-event sync), NOT the whole architecture.
4. **PROJECTS / FEATURED** — the strong genuine projects, strongest first; no unverified README numbers.
5. **SKILLS** — the full genuine list (grouped, up to 100), and call out the **3 to pin** (highest search value across the target roles).
