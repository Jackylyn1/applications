...
- **Main context** (universal branding + voice) and the **LinkedIn context**.
- Facts come ONLY from `career-kb/profile.json` (read it, incl. `personal.linkedin_headline_current`, work_experience, github_repositories, skills, role_skill_map, career_goals). Never invent.

## WHAT TO PRODUCE (your final message = the optimization brief)
1. **Current-state audit:** what the current headline/positioning does well and where it loses recruiter searches or trust; concrete over-/understatements to fix (e.g. Gastro IT = core contributor, not sole architect; Laravel Pivot = OSS contribution; no unverified numbers).
2. **Multi-role keyword strategy:** one umbrella that covers as many target roles as possible (AI Software Developer, Software Architect, Solution Architect, PHP Developer as baseline; AI Consultant partial). The exact high-value search keywords/phrases recruiters use for those roles (EN + DE variants) that are genuinely hers.
...
4. **Skills plan:** the full genuine skills list (up to 50), which 3 to pin, and grouping/order by search value.
