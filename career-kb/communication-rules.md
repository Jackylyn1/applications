...
### Preferred tone
- Professional but approachable; in German professionell, aber locker: relaxed enough to read like a person, never stiff, never chummy. It reads pleasantly and lets a recruiter orient in seconds. A sentence that serves only the reading experience goes.
...
- Explain trade-offs and reasoning in prose and in interviews, never inside a bullet point. A bullet is too short to carry a "because" (see *Bullet points*).
...
- **One bullet per unit of work, never per fact.** A unit of work is a project, a component or an initiative: the starter-kit optimisation, the external API, the test automation. Everything belonging to it, including its several outcomes, stays in that one bullet.
  ...
  - Wrong: the same content as four bullets. Splitting inflates the list and destroys the information that this was one piece of work with several results.
...
- **One to two lines each** (~200 characters). Longer carries reasoning, which belongs in prose. Shorter than a unit of work means it was split.
...
- **Numbers wherever `profile.json` has one** — throughput, runtime, page count, tenant count, coverage, duration. A real number outranks any adjective. Where none exists, state the effect qualitatively and ask; never estimate.
  - **A count of something abstract needs an example beside it.** "10 Zielbegriffe", "14 Domains", "50 Ressourcen", "32 Kanäle" are unreadable alone; the reader cannot judge the scale. Name one or two actual items in brackets: "rund 10 Zielbegriffe mit lokalem Onlinemarketingbezug (z. B. Webseite Gelsenkirchen)". Concrete units (80 %, 5 Stunden, 227 Tests) already carry their scale.
- **Cut filler.** Delete "responsible for", "involved in", "helped to", "worked on", "tasked with", "various", "several", "successfully", "state-of-the-art", standalone "modern", and any "in order to". Drop the leading "I" and articles where the meaning survives.
  - **Honesty outranks this ban.** Where a weak verb is the accurate one — "worked on" for the Docker Compose dev/CI environment she did not build — drop the bullet, never swap in a stronger verb. A weak verb signals that the item does not carry its own bullet.
- **No terminal punctuation**, applied consistently across all bullets in a document.
- No narrator commentary and no negation inside bullets. A bullet has no room to recover from either.

### The person description stays prose (hard)
The CV summary, the LinkedIn About section and the cover-letter body are flowing text, never bullets. They are the place that reads as a person rather than an index. All three carry the same register, and the LinkedIn About text (`output/LinkedIn_Profile_optimized_de.txt`, section 2) is the working model.
...
- No highlighting of gaps. "2021–2024 and since 2025" spells out an employment gap that plain date ranges carry silently. Write the roles with their own dates and stop.
...
- The "keep optimizing" idea without its pragmatic qualifier, and especially in German: "ich höre selten bei der ersten Lösung auf, die funktioniert" reads as overengineering. The English pattern with "if time allows it" is allowed. In German either carry an equivalent constraint ("wenn es die Zeit zulässt") or show the behaviour through a result.
