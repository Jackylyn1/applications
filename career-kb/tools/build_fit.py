...
def page2_lines(pdf):
    out = subprocess.run(
        ['pdftotext', '-f', '2', '-l', '2', pdf, '-'], capture_output=True, text=True, check=False
    ).stdout
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for scale, drop in STEPS:
            ...
            print(
                f"  try scale={scale} drop={drop}: pages={pages} page2_lines={p2} "
                f"{'OK' if ok else '...'}"
            )
            if ok:
        ...
        if chosen is None:
            ...
            print(
                f"  NOTE: one page would need cutting >3 bullets; kept intact "
    ...
    print(
        f"wrote {args.out} + {os.path.basename(final_pdf)} "
